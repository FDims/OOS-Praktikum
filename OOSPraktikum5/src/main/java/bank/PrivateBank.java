...
public class PrivateBank implements Bank {
            ...
            if(file.length()>0) {
                try {
                    ...
                    e.printStackTrace();
                         ...
                         TransactionAttributeException | IncomingException | OutgoingException e) {
    ...
    public void deleteAccount(String account) throws AccountDoesNotExistException, IOException{
        if(!accountsToTransactions.containsKey(account)){
            ...
            Path path = Path.of(this.getPath() + "/" + account + ".json");
            Files.deleteIfExists(path);
            System.out.println("account with the name "+account+" in the bank "+name+" is deleted!");
        }
    }
